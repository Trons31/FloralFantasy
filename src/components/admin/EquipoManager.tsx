...
export default function EquipoManager({ members: init }: { members: Member[] }) {
  ...
  const handleSave = async () => {
    ...
    if (form.pin.length !== 4) {
      toast.error("PIN de 4 dígitos requerido");
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    await fetch(`/api/equipo/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <>
      ...
      <ResponsiveModal open={showForm} onClose={() => setShowForm(false)} title="Nuevo miembro" panelClassName="md:max-w-sm">
      ...
      <ResponsiveModal open={!!editing} onClose={() => setEditing(null)} title="Editar miembro" panelClassName="md:max-w-sm">
        {editing && (
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Nombre *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Nuevo PIN <span className="text-gray-400 font-normal">(opcional)</span>
              </label>
                ...
                {[0, 1, 2, 3].map(i => (
                  <input
                    ...
                    onChange={e => {
                      ...
                      const next = [...editForm.pinDigits];
                      next[i] = e.target.value;
                      setEditForm(p => ({ ...p, pinDigits: next, pin: next.join("") }));
                      if (e.target.value && i < 3) document.getElementById(`epin-${i + 1}`)?.focus();
                    }}
                    onKeyDown={e => {
                      if (e.key === "Backspace" && !editForm.pinDigits[i] && i > 0) {
                        document.getElementById(`epin-${i - 1}`)?.focus();
                      }
                    }}
                    className="w-14 h-14 text-center text-2xl font-bold border-2 border-gray-200 rounded-xl focus:border-primary-500 focus:outline-none transition-colors"
                  />
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={() => setEditing(null)} className="flex-1 border border-gray-200 text-gray-600 py-3.5 rounded-xl text-sm font-medium">
                Cancelar
              </button>
              <button
                onClick={handleEdit}
