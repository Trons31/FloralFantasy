...
import { z } from "zod";
import { RiArrowRightLine, RiTimeLine, RiShoppingBagLine, RiTruckLine, RiShieldCheckLine, RiFlowerLine } from "react-icons/ri";
...
export default function CheckoutPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Construir URL con signature de integridad generado en el servidor
      const params = new URLSearchParams({
        "public-key":       pubKey || "",
        "currency":         "COP",
        "amount-in-cents":  String(order.amountInCents),
        "reference":        order.id,
        "redirect-url":     `${appUrl}/checkout/confirmacion`,
      });

      // Agregar signature si está disponible
      if (order.wompiSignature) {
        params.set("signature:integrity", order.wompiSignature);
      }
