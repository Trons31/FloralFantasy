      ...
      <header className="sticky top-0 z-20 border-b border-slate-100 bg-white/95 px-4 py-4 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-xl items-center justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-amber-50 text-amber-500"><RiFlowerLine size={26} /></span>
            <div className="min-w-0">
              <h1 className="truncate text-lg font-bold leading-tight">Preparador</h1>
              <p className="truncate text-sm text-slate-500">{user.name}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button type="button" onClick={load} className="grid h-10 w-10 place-items-center rounded-xl text-slate-600 hover:bg-slate-50" aria-label="Actualizar pedidos">
  ...
  return (
    <article className={`overflow-hidden rounded-3xl border bg-white shadow-[0_8px_30px_rgba(15,23,42,.06)] ${tab === "PROCESSING" ? "border-amber-200" : tab === "READY" ? "border-emerald-100" : "border-primary-100"}`}>
      <div className="p-4">
        ...
        <div className="mt-4 grid grid-cols-[106px_1fr] gap-3">
          <div className="h-[120px] overflow-hidden rounded-2xl bg-slate-50">
      ...
      <div className="mx-4 rounded-2xl bg-slate-50 p-1">
        <div className="grid grid-cols-3">
          {(["summary", "flowers", "notes"] as const).map(value => (
            <button key={value} type="button" onClick={() => setSection(value)} className={`h-10 rounded-xl text-[11px] font-semibold transition ${section === value ? "bg-white text-primary-500 shadow-sm" : "text-slate-500"}`}>
      ...
      <div className="p-3.5">
        ...
        {tab !== "READY" && (
          <button type="button" onClick={() => onAction(order.id, tab === "PAID" ? "PROCESSING" : "READY")} disabled={updating} className="mt-4 inline-flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-[#ff8a00] to-[#ff9f0a] px-4 text-sm font-bold text-white shadow-[0_12px_28px_rgba(255,138,0,.28)] transition active:scale-[0.99] disabled:opacity-60">
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5">
      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3.5">
        ...
        <p className="mt-3 text-[9px] text-slate-400">Tamaño del ramo</p>
        <strong className="mt-1 block text-xs">{sizeLabel}</strong>
      </div>
      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3.5">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-rose-50 text-primary-500"><RiCalendarLine /></span>
        <p className="mt-3 text-[9px] text-slate-400">Entrega estimada</p>
        <strong className="mt-1 block text-xs">{order.estimatedTime}</strong>
      </div>
      <div className="col-span-2 rounded-2xl border border-slate-100 bg-slate-50 p-3.5">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-primary-50 text-primary-500"><RiStackLine /></span>
        <p className="mt-2.5 text-[9px] text-slate-400">Productos</p>
        <strong className="mt-1 block text-xs">{totalProducts} producto{totalProducts === 1 ? "" : "s"}</strong>
      </div>
    </div>
  );
}
function FlowersList({ flowers, extras, processing = false, compact = false }: { flowers: FlowerLine[]; extras: ExtraFlower[]; processing?: boolean; compact?: boolean }) {
  return (
    <div className="space-y-2.5">
      {!compact && <h4 className="text-xs font-bold text-slate-500">Flores del ramo</h4>}
      {flowers.map(flower => (
        <div key={flower.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-2.5 shadow-sm">
          {processing && <RiCheckboxCircleFill className="shrink-0 text-emerald-500" size={23} />}
          <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-rose-50">
      ...
      {extras.map(flower => (
        <div key={flower.id} className="flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/40 p-2.5">
          <RiAddCircleLine className="text-emerald-500" /><div className="flex-1"><strong className="text-[11px]">{flower.name}</strong><small className="block text-[8px] text-emerald-600">Flor adicional</small></div><strong className="text-xs">{flower.quantity}</strong>
        </div>
      ))}
      {!flowers.length && !extras.length && <p className="py-5 text-center text-[11px] text-slate-400">Sin flores detalladas.</p>}
    </div>
  );
}
...
function Notes({ order, addons }: { order: Order; addons: Array<{ addon: { name: string } }> }) {
  return (
    <div className="space-y-2.5">
      ...
      {addons.length > 0 && <div className="rounded-2xl border border-violet-100 bg-violet-50 p-4"><h4 className="text-[11px] font-bold text-violet-700">Adicionales</h4><p className="mt-2 text-[11px] text-slate-600">{addons.map(entry => entry.addon.name).join(", ")}</p></div>}
    </div>
  );
}
...
function ElapsedTimer({ startedAt }: { startedAt: string }) {
  ...
  return (
    <div className="mt-4 flex items-center justify-between rounded-2xl border border-orange-100 bg-orange-50/70 px-3.5 py-3">
      <div className="flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-orange-500 shadow-sm"><RiTimeLine size={21} /></span>
        <div>
          <p className="text-[10px] text-slate-500">Tiempo transcurrido</p>
          <strong className="text-[20px] leading-none text-orange-500">{String(minutes).padStart(2, "0")}:{String(remaining).padStart(2, "0")}</strong>
        </div>
      </div>
    </div>
  );
