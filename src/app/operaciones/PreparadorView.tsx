    ...
    <div className="min-h-screen bg-amber-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {loading ? (
          ...
          <>
            {/* PAID — nuevos por preparar */}
            {paid.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500"/>
                  <h2 className="font-bold text-gray-700 text-sm uppercase tracking-wide">Por preparar ({paid.length})</h2>
                </div>
                <div className="space-y-3">
                  {paid.map(order => (
                    <OrderCard key={order.id} order={order} updating={updating}
                      action={{ label:"Empezar a preparar", status:"PROCESSING", color:"bg-amber-500 hover:bg-amber-600" }}
            ...
            {processing.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-bold text-gray-700 text-sm uppercase tracking-wide">En preparación ({processing.length})</h2>
                </div>
                <div className="space-y-3">
                  {processing.map(order => (
                    <OrderCard key={order.id} order={order} updating={updating}
                      action={{ label:"Marcar como listo", status:"READY", color:"bg-green-500 hover:bg-green-600" }}
                      onAction={updateStatus}/>
                  ))}
                </div>
              </div>
